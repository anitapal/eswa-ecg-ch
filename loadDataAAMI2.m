...
%
% pca = 1 => faz o PCA com 99%
% pca = 0 ignora PCA
%
function [p1d p1t p2d p2t] = loadDataAAMI2(pca, file_path, trainset, testset)
...
pos = find(amostra(:,tam)==70);  % F -> F -> VEB para aami2
F = amostra(pos,1:tam-1);
sz = size(F,1);
tF = repmat([0 0 1 ],sz,1); 

pos = find(amostra(:,tam)==106);  % j -> N
j  = amostra(pos,1:tam-1);
sz = size(j,1);
tj = repmat([1 0 0],sz,1); 

pos = find(amostra(:,tam)==69);  % E -> VEB
E  = amostra(pos,1:tam-1);
sz = size(E,1);
tE = repmat([0 0 1],sz,1); 

pos = find(amostra(:,tam)==74);  % J -> SVEB
J  = amostra(pos,1:tam-1);
sz = size(J,1);
tJ = repmat([0 1 0],sz,1); 

pos = find(amostra(:,tam)==101);  % e -> N
e  = amostra(pos,1:tam-1);
sz = size(e,1);
te = repmat([1 0 0],sz,1); 

pos = find(amostra(:,tam)==83);  % S -> SVEB
S  = amostra(pos,1:tam-1);
sz = size(S,1);
tS = repmat([0 1 0],sz,1); 

data = [N;L;R;A;V;a;F;S;j;E;J;e];
target = [tN;tL;tR;tA;tV;ta;tF;tS;tj;tE;tJ;te];
end
