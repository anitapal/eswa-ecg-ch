function [fs,history]=sequencialfsSVM(featureSet, modelo)
% Faz uma busca para frente testando a sensibilidade do SVM
%
% >> featureSet: conjunto de características
% Pode assumir os valores:
% 'YuCN','YuFourier','YuFourier_A2A4', '2005Chazal', '2005Guler',etc...
%
%
%
%
...
for k=1:size(registers,1) % numero de registros
    ...
    if nargin < 2  % apenas gera modelos se nenhum foi passado por parâmetro
        ...
        cmd = ['-w1 1 -w2 50 -w3 10 -w4 100 -c ',num2str(param_c),' -g ',num2str(param_g)]; 
...
%fprintf('\n **** ATENCAO : INICIALIZA PARA TESTE: ****REMOVER!!!!! \n')
%keep(1) = 1; %% ATENCAO : INICIALIZA PARA TESTE: ****REMOVER!!!!!!
%keep(5) = 1;
%keep(23) = 1;
%keep(24) = 1;
%keep(25) = 1;
...
for i=1:size(p2d,2)
   ...
   % acha a maior acurácia média para todos os pacientes
    acc_mean = mean(acc,1);
    acc_sorted = sort(acc_mean,'descend');
    
    tt=1;    
    index = find(acc_mean == acc_sorted(tt));
    
    % não pega um indice se ele já foi o escolhid
    while keep(1,index(1))==1 && tt<size(keep,2) 
        tt = tt+1;
        index = find(acc_mean == acc_sorted(tt));
    end
    
    if(acc_mean(index(1)) > (best_acc_mean + 0.005 * best_acc_mean)) % mantém se o resultado for maior que acc médio + 0.5%
        ...
        % faz o teste em lopoDS1 e DS1->DS2
        %classify_ds1_ds2(featureSet,'SVM','AAMI',0,keep,i);
        %classify_leave_one_p_out(featureSet,'SVM','AAMI',0,keep,i);
    else
