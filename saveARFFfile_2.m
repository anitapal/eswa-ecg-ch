...
% ATENCAO : salva em formato AAMI2, proposto por Llamedo et al (2001).
%
function [p1d p1t p2d p2t] = saveARFFfile_2(featureSet, featSel, smote_S_V)
...
if (~isempty(featSel))
    ...
    folder_ds1 = ['.\WEKA\' featureSet '_FS' num2str(numfs) '_' '_AAMI2_DS1.arff'];
else
    folder_ds1 = ['.\WEKA\' featureSet '_AAMI2_DS1.arff'];
end
...
if (~isempty(featSel))
    %numfs = size(featSel,2);
    numfs = size(find(featSel==1),2);
    folder_ds2 = ['.\WEKA\' featureSet '_FS' num2str(numfs) '_' '_AAMI2_DS2.arff'];
    
else
    folder_ds2 = ['.\WEKA\' featureSet '_AAMI2_DS2.arff'];
end
