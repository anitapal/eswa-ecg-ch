function [fs,history]=sequencialfsSVM_hierarq_1(featureSet, modelo)

path(path,'svm')

s = char(featureSet);
fileNamed = ['results\',s,'_FeatureSelection_hierarq_1.tex'];
...
for k=1:size(registers,1) % numero de registros
    ...
    %p1d = instancias
    %p1t = label do treino
    [p1d p1t p2d p2t] = loadData_CH_step_1(0,feat_folder,train_ds,test_ds);
    ...
    if nargin < 2  % apenas gera modelos se nenhum foi passado por parâmetro
        ...
        cmd = ['-w1 1 -w2 10 -c ',num2str(param_c),' -g ',num2str(param_g)]; 
